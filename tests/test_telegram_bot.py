...
def test_message_forwarded_to_polaris(mock_post):
    ...
    mock_post.side_effect = lambda *args, **kwargs: mock_polaris_response if "inference" in args[0] else mock_telegram_response
    ...
    if mock_post.call_args_list:
        ...
        assert "/inference/" in polaris_call_url, f"Chamada inesperada: {polaris_call_url}"
        assert polaris_call_data == {"prompt": "Qual a capital da França?", "session_id": "456"}, f"Payload inesperado: {polaris_call_data}"
